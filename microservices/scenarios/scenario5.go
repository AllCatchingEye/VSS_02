...
func Scene5(rdb *redis.Client, nc *nats.Conn) bool {
	...
	// build connection to shipment service
	...
	log.Printf("Retour order with ID: %v", rRetour.GetSuccess())

	fmt.Println("Done")
	return true
}

func fillStore5(rdb *redis.Client, nc *nats.Conn) {
	// set context
	...
	fmt.Println("FillStore finished")
}
